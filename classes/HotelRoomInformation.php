...
class HotelRoomInformation extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_category_normal' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            'id_category_festival' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
    ...
    public function getOnlineRoomCount($id_hotel) {
        $hotel_event_calendar = new HotelEventCalendar();
        if (!$hotel_event_calendar->isFestivalMode()) {
            $query = 'id_category_normal=1';
        } else {
            $query = 'id_category_festival=1';
        }
        return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'htl_room_information` where `id_hotel`='.$id_hotel);
    }
}
